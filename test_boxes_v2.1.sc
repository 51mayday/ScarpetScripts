...
make_test_boxes(pos1, pos2, count, unit, option) -> (
    ...
    boxes = [];
    while(length(keys(itemCounts)) > 0, floor(length(itemCounts)*count * if(unit == 'stacks', 64, 1) / 27) + 1,
        box = []; 
...
make_test_boxes_file(pos1, pos2, count, unit, filenames) -> (
    ...
    boxes = [];
    while(length(itemCounts) > 0, floor(length(itemCounts)*count * if(unit == 'stacks', 64, 1) / 27) + 1,
        box = []; 
...
make_results_boxes(pos1, pos2, count, unit, option) -> (
    ...
    for(keys(itemCounts), 
        item = _;
        while(itemCounts:item > 0, floor(count / stack_limit(item:0) * if(unit == 'stacks', 64, 1) / 27) + 1, 
            box = [];
