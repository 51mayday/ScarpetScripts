// Inventory Analyzer v1.3.1 by 51 Mayday - for MC 1.21+
...
// Returns results as .csv, .json, or .txt files and prints

// v1.3.1 changelog:
// Fixed itemtypes data to output issue

// v1.3 changelog:
// Added options for output file format: .csv, .json, or .txt
...
init(startPos, endPos, countMode, outputType, filename) -> (
    global_checked = {};
...
countSlots(startPos, endPos, outputType, filename) -> (
    ...
    , if(outputType == 'json',
        return(scarpet_json(slots, filename));
    , if(outputType == 'text',
        return(julia_format(slots, filename));
    );););
);
...
countItems(startPos, endPos, outputType, filename) -> (
    ...
    , if(outputType == 'json',
        return(scarpet_json(item_count, filename));
    , if(outputType == 'text',
        return(julia_format(item_count, filename));
    );););
);
...
julia_format(list, filename) -> (
    // save a csv-formatted .txt file
    // overwrites old file

    delete_file(str('%s.txt', filename), 'shared_any');

    if(type(list) == 'map', // itemtype mode
        output = 'itemtype, count\n';
        for(list,
            output += str('%s, %d\n', _, list:_);
        );
    , if(type(list) == 'list', // slots mode
        output = 'itemtype, count, stackability, contents: [contents, stackability, count, itemtype]\n';
        for(list,
            output += str('%s, %d, %d, %s\n', _:'itemtype', _:'count', _:'stackability', _:'contents');
        );
    ...
    write_file(str('%s.txt', filename), 'shared_any', output);
    print(player(), format('wi Saved to: \\scripts\\shared\\' + filename + '.txt'));

    return(output);
);

scarpet_json(list, filename) -> (
    print(player(), list);
