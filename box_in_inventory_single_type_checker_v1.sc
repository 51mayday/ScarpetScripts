// Box in Inventory Single Type Checker V1
...
record(origin_pos) -> (
    ...
    if(inventory_has_items(origin_pos), 
        ...
        for(slots,
            if(_ == null || _:2 ~ 'already_checked', continue(););
            box_type = '';
            slotData = _;
            slotIndex = _i;
            if(item_tags(_:0, 'c:shulker_boxes'), 
                if(slotData:2 == null || !(slotData:2 ~ 'BlockEntityTag'), 
                    slotData:2 = {}; 
                    print(player(), 'Fail: empty box'); 
                    global_counter:'fail':'empty' = global_counter:'fail':'empty' + 1;
                    , // else
                    clean_box = true;
                    for(parse_nbt(slotData:2):'BlockEntityTag':'Items',
                        item_type = _:'id';
                ...
                slotData:2:'already_checked' = true;
                inventory_set(origin_pos, slotIndex, slotData:1, slotData:0, slotData:2);
            , // else
                print(player(), 'Fail: non-box');
                if(slotData:2 == null, slotData:2 = {});
                slotData:2:'already_checked' = true;
                inventory_set(origin_pos, slotIndex, slotData:1, slotData:0, slotData:2);
...
clean(clean_pos) -> (
    if(inventory_has_items(clean_pos),
        ...
        for(slots,
            if(_ == null, continue(););
            if(_:2 ~ 'already_checked',
                delete(_:2, 'already_checked');
                inventory_set(clean_pos, _i, _:1, _:0, _:2);
            )
        );
    );
