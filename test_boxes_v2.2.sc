// Test Boxes v2.2_dev by 51mayday
// For MC versions 1.20.5 and above
...
__config() -> {
    'commands' -> {
        'test <from_pos> <to_pos> <count> <unit> <option>' -> 'make_test_boxes',
        'results <from_pos> <to_pos> <count> <unit> <option>' -> 'make_results_boxes',
        'test <from_pos> <to_pos> <count> <unit> file <filenames> ' -> 'make_test_boxes_file',
        'results <from_pos> <to_pos> <count> <unit> file <filenames> ' -> 'make_results_boxes_file'
    },
    'arguments' -> {
        'option' -> {
            'type' -> 'term',
...
_make_box(boxItems) -> (
    nbt = '{components:{"minecraft:container":[';
    for(boxItems, 
        nbt += str('{item:{count:%db,id:"minecraft:%s"%s},slot:%db}%s', _:1, _:0:0, if(_:0:1 != null, str(',%s', _:0:1), ''), _i, if(_i < length(boxItems) - 1, ',', ''));
    );
    nbt += ']},count:1, id:"minecraft:white_shulker_box"}';
    return(nbt);
);

make_test_boxes(pos1, pos2, count, unit, option) -> (
    items = _get_item_list(option);
    itemCounts = reduce(items, x = _a; put(x, _, count * if(unit == 'stacks', stack_limit(_:0), 1)); _a = x;, {});
...
make_test_boxes_file(pos1, pos2, count, unit, filenames) -> (
    ...
    items = map(items, if(_isInvalidItem(_:0), ['tropical_fish', str('components:{"minecraft:custom_name":"%s"}', _:0)], [_:0, _:1]));
    itemCounts = reduce(items, x = _a; put(x, _, count * if(unit == 'stacks', stack_limit(_:0), 1)); _a = x;, {});
...
make_results_boxes(pos1, pos2, count, unit, option) -> (
    items = _get_item_list(option);
    itemCounts = reduce(items, x = _a; put(x, _, count * if(unit == 'stacks', stack_limit(_:0), 1)); _a = x;, {});
...
make_results_boxes_file(pos1, pos2, count, unit, filenames) -> (
    item_lists = split(' ', filenames);
    items = [];
