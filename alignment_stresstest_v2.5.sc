...
// In case of mismatched position or mismatched motion, it will count it as a failure and log/report the failed location and motion
...
record(spawn_pos, range, age, item) -> (
	...
	if(global_counter:item:'has_recorded', 
		for(sample_items,
			...
			is_pos_ok = all(_ ~ 'pos' - global_counter:item_id:'pos':((_ ~ 'id') % 4), abs(_) <= global_epsilon);
			is_motion_ok = all(_ ~ 'motion' - global_counter:item_id:'motion':((_ ~ 'id') % 4), abs(_) <= global_epsilon);
	...
	, // else
		for(sample_items,
			global_counter:item:'pos':((_ ~ 'id') % 4) = _ ~ 'pos';
			global_counter:item:'motion':((_ ~ 'id') % 4) = _ ~ 'motion';

			if(all(global_counter:item:'pos', _ != null),
				...
				print(format(
					...
					'gi Starting test', 
					'f \n»', 
					'gi  Setting expected positions: \n', 
					str('  0: %s\n 1: %s\n 2: %s\n 3: %s\n', global_counter:item:'pos':0, global_counter:item:'pos':1, global_counter:item:'pos':2, global_counter:item:'pos':3),
					'f \n»', 
					'gi  Setting expected motions: \n', 
					str('  0: %s\n 1: %s\n 2: %s\n 3: %s\n', global_counter:item:'motion':0, global_counter:item:'motion':1, global_counter:item:'motion':2, global_counter:item:'motion':3),
				));
...
report(item_id, item) -> (
	print(str('%s (mod4: %d) failed at: \n %s with motion: \n %s', item_id, (item ~ 'id') % 4, item ~ 'pos', item ~ 'motion'));
);