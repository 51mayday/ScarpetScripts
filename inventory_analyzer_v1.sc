// Inventory Analyzer v1 by 51 Mayday
...
// The second returns the contents of each slot (including any items in shulker boxes in inventories).
// Returns results as csv-formatted .txt files (scarpet doesn't support working with csv files directly :[ )
// THERE MAY BE BUGS
...
csv(list, filename) -> (
    ...
    delete_file(filename, 'shared_text');
    ...
    write_file(str(filename), 'shared_text', output); // change to shared_csv if scarpet ever supports it
    print(player(), format('wi Saved to: \\scripts\\shared\\' + filename + '.txt'));
    print(player(), format(
                        'gi  Unfortunately, .csv files are not supported by scarpet at the time of writing.',
                        'gi  You will have to rename ' + str(filename),
                        'gbi .txt',
                        'gi  to ' + str(filename),
                        'gbi .csv'
        ));
